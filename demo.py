...
def get_arguments():
    ...
    parser.add_argument(
        "--img_name",
        type=str,
        default="GOPR0364_frame_000939_rgb_anon.png",
        help="Image to be used for demo",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="./light-effects-output/",
    ...
    parser.add_argument(
        "--load_model", type=str, default=None, help="model to initialize with"
    )
    parser.add_argument(
        "--load_size",
        type=str,
    ...
    parser.add_argument(
        "--iters", type=int, default=60, help="No of iterations to train the model."
    )
    parser.add_argument(
        "--learning_rate", type=float, default=1e-4, help="Learning rate for the model."
    )
...
def get_LFHF(image, rad_list=[4, 8, 16, 32], eps_list=[0.001, 0.0001]):
    ...
    img_lf, img_hf = decomposition(
        guide=image, inp=image, rad_list=rad_list, eps_list=eps_list
    )
...
class Vgg16ExDark(torch.nn.Module):
    def __init__(self, load_model=None, requires_grad=False):
        ...
        if load_model is None:
            ...
            model_state_dict = {
                k[16:]: v for k, v in model_state_dict.items() if k[16:] in model_dict
            }
...
class PerceptualLossVgg16ExDark(nn.Module):
    def __init__(
        self, vgg=None, load_model=None, weights=None, indices=None, normalize=True
    ):
...
def demo(args, dle_net, optimizer_dle_net, inputs):
    ...
    cc_loss = (
        F.l1_loss(dle_pred[:, 0:1, :, :], dle_pred_cc)
        + F.l1_loss(dle_pred[:, 1:2, :, :], dle_pred_cc)
        + F.l1_loss(dle_pred[:, 2:3, :, :], dle_pred_cc)
    ) * (
        1 / 3
